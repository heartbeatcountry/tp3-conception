...
public class FilmQueryServiceTests : GenericServiceTests<FilmQueryService>
{
    ...
    [Test]
    public async Task ObtenirFilmsAssistesParUtilisateur_WhenUtilisateurExist_ShouldReturnAllFilmsForThisUser()
    {
        // Arrange

        Guid idUtilisateur = Guid.NewGuid();

        IEnumerable<IFilm> films = new List<IFilm>
        {
            Mock.Of<IFilm>(f => f.Id == Guid.NewGuid()),
            Mock.Of<IFilm>(f => f.Id == Guid.NewGuid()),
            Mock.Of<IFilm>(f => f.Id == Guid.NewGuid())
        ...
        IEnumerable<IProjection> projections = new List<IProjection>
        {
            Mock.Of<IProjection>(p => p.IdFilm == films.ElementAt(0).Id),
            Mock.Of<IProjection>(p => p.IdFilm == films.ElementAt(1).Id),
            Mock.Of<IProjection>(p => p.IdFilm == films.ElementAt(2).Id)
        };

        IEnumerable<IBillet> billets =
        [
            Mock.Of<IBillet>(b => b.IdUtilisateur == Guid.NewGuid() && b.IdProjection == projections.ElementAt(0).Id),
            Mock.Of<IBillet>(b => b.IdUtilisateur == Guid.NewGuid() && b.IdProjection == projections.ElementAt(1).Id),
            Mock.Of<IBillet>(b => b.IdUtilisateur == Guid.NewGuid() && b.IdProjection == projections.ElementAt(2).Id)
        ];


        // Configuration des mocks pour simuler le comportement attendu
        BilletRepositoryMock.Setup(r => r.ObtenirTousAsync(It.IsAny<Expression<Func<IBillet, bool>>>(),
            It.IsAny<Func<IQueryable<IBillet>, IOrderedQueryable<IBillet>>>())).ReturnsAsync(billets);

        ProjectionRepositoryMock.Setup(r => r.ObtenirTousAsync(It.IsAny<Expression<Func<IProjection, bool>>>(),
            It.IsAny<Func<IQueryable<IProjection>, IOrderedQueryable<IProjection>>>())).ReturnsAsync(projections);

        FilmRepositoryMock.Setup(r => r.ObtenirTousAsync(It.IsAny<Expression<Func<IFilm, bool>>>(),
            It.IsAny<Func<IQueryable<IFilm>, IOrderedQueryable<IFilm>>>())).ReturnsAsync(films);
